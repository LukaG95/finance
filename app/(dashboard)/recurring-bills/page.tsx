...
export default async function RecurringBillsPage({ searchParams }: any ) {
  ...
  const dueSoon = bills.filter(b => b.dueDay >= today && b.dueDay - today <= 3);
  const upcoming = bills.filter(b => b.dueDay - today > 3);

  const totalAmount = bills.reduce((sum, b) => sum + b.amount, 0);
  const totalUpcomingAmount = [...dueSoon, ...upcoming].reduce((sum, b) => sum + b.amount, 0);
          ...
          <Card className="!p-250">
            ...
            <div className="mt-250 text-preset-5 flex flex-col divide-y divide-grey-500/15">
              ...
              <div className="flex justify-between py-200">
                ...
                <span className="text-preset-5-bold text-grey-900">
                  {upcoming.length + dueSoon.length} ({formatCurrency(totalUpcomingAmount)})
                </span>
              </div>
        ...
        <Card className="flex flex-col gap-300 h-max !py-300 !px-250 sm:!p-400">
          ...
          <div className="overflow-hidden rounded-[8px]">
            ...
            {bills.map((bill, i) => {
              ...
              const isDueSoon = bill.dueDay >= today && bill.dueDay - today <= 3;
                ...
                <div key={bill._id.toString()} className="border-t border-grey-100">
                  ...
                  <div className={`flex flex-col gap-150 py-250 sm:hidden ${isLast ? 'pb-0' : ''}`}>
                    ...
                    <div className="flex items-center gap-200">
                      ...
                      />
                      <span className="text-grey-900 text-preset-4-bold">{bill.title}</span>
                    </div>
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <div className={`text-right font-semibold ${isDueSoon ? 'text-red-600' : 'text-grey-900'}`}>
                  ...
                  <div className={`hidden sm:grid grid-cols-[5fr_1fr_100px] items-center 1350:px-200 py-250 text-preset-4 hover:bg-beige-50 transition ${isLast ? 'pb-0' : ''}`}>
                    ...
                    <div className={`text-right text-preset-4-bold ${isDueSoon ? 'text-red-600' : 'text-grey-900'}`}>
                      {formatCurrency(bill.amount)}
                    </div>
                  </div>
                </div>
